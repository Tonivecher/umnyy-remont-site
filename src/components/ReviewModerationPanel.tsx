...
import { formatReviewDate, ReviewStatus, StoredReview } from '@/src/utils/reviews';
...
export const ReviewModerationPanel: React.FC<ReviewModerationPanelProps> = ({ onReviewsUpdated }) => {
  ...
  const loadReviews = async (rawPassword = password, filter = activeFilter) => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : 'Не удалось загрузить отзывы для модерации.');
  ...
  const handleAction = async (reviewId: string, action: 'approve' | 'reject' | 'pending' | 'delete') => {
    ...
    try {
      ...
      setError(actionError instanceof Error ? actionError.message : 'Не удалось обновить статус отзыва.');
    } finally {
  ...
  return (
    <div className="mb-16 border border-white/10 bg-white/4 p-8 md:p-10 rounded-sm">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div>
          <span className="text-[10px] uppercase tracking-[0.3em] opacity-40 block mb-3">Скрытый режим</span>
          ...
          <p className="text-sm opacity-60 mt-4 max-w-2xl leading-relaxed">
            Все новые отзывы сначала попадают в очередь. Публично видны только отзывы со статусом «Опубликован».
          </p>
        </div>
      ...
      {!isUnlocked ? (
        <form onSubmit={handleUnlock} className="mt-8 flex flex-col md:flex-row gap-4">
          <input
            ...
            className="flex-1 bg-transparent border border-white/15 px-5 py-4 outline-none focus:border-white/40 transition-colors"
        ...
        <>
          <div className="mt-8 flex flex-wrap gap-3">
            {filterOrder.map((filter) => (
              <button
                ...
                className={`px-4 py-3 border text-[10px] uppercase tracking-[0.25em] transition-colors ${
                  activeFilter === filter
          ...
          <div className="mt-8 space-y-5">
            {reviews.length === 0 ? (
              <div className="border border-white/8 p-6 text-sm opacity-50">
                Для выбранного фильтра отзывов пока нет.
              </div>
            ) : (
              reviews.map((review) => (
                <article key={review.id} className="border border-white/8 p-6 md:p-8">
                    ...
                    <div className="flex flex-wrap gap-3 md:max-w-[280px]">
                      ...
                      {review.status !== 'pending' ? (
                        <button
                          ...
                          disabled={actionReviewId === review.id}
                          className="px-4 py-3 border border-white/15 text-[10px] uppercase tracking-[0.25em] text-white/75 hover:text-white hover:border-white/30 transition-colors disabled:opacity-50"
                        >
                          <span className="inline-flex items-center gap-2">
                            <Clock3 size={14} />
                            Вернуть в очередь
                      ...
                      <button
                        ...
                        disabled={actionReviewId === review.id}
                        className="px-4 py-3 border border-red-400/20 text-[10px] uppercase tracking-[0.25em] text-red-100/70 hover:text-red-100 hover:border-red-400/40 transition-colors disabled:opacity-50"
                      >
                        <span className="inline-flex items-center gap-2">
                          <Trash2 size={14} />
                          Удалить
